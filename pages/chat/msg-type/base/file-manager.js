...
export default class FileManager {
    ...
    async _sendFileMsg({content, duration, type}) {
        const temp = this._page.imOperator.createNormalChatItem({type, body:content, duration});
    ...
    async uploadFileAndSend({content, duration, type, itemIndex}) {
        ...
        const {msg} = await this._page.sendMsg({
            content: this._page.imOperator.createChatItemContent({type, body: url, duration}),
            itemIndex,
        });
